...
public:
    ...
    Client(const char* nume,const int id_client,const double buget,const char* telefon):id_client(id_client),buget(buget)
    {
...
public:
    ...
    void proprietar_rezervare()const
    {
        int id=-1,poz=-1,nr=0;
        for(int i=0; i<numar_clienti; i++)
            if(clienti[i].telefon_valid())
            {
                ...
                {
                    poz=i;
                    id=clienti[i].get_id_client();
                }
            }
        if(nr==1)
            std::cout<<"Persoana care a facut aceasta rezervare este "<<clienti[poz].get_nume()<<" avand id-ul de client "<<id<<".\n";
        else if(nr==0)
            std::cout<<"Rezervare eronata!Niciun client nu are numar de telefon valid!"<<'\n';
        else
            std::cout<<"Nu se cunoaste cine a facut rezervarea.Informatiile rezervarii "<<id_rezervare<<" au fost comunicate de catre mai multi clienti!"<<'\n';
    }
...
{///Creare rezervare 1
    ...
    Client c1("Ion Popescu",7,1000.87,"Necunoscut");
    Client c2("Maria Ionescu",3,499.99,"Necunoscut");
    Client c3("Eduard Ungureanu",8,285,"0771170533");
    c1.marire_buget(50);
...
///Creare rezervare 2
    ...
    Client c4("Popescu Mihai",1,121,"0744561232");
    c4.marire_buget(89);
    Client c5("Stan Ana",6,2000,"0711223453");
...
///Creare rezervare 4
    ...
    Client c8("Matei Ioana",10,800.01,"0773451977");
    Client c9("Stefania Adelina",22,821,"Necunoscut");
    Client c10("Badea Ionut",12,911,"Necunoscut");
    Client c11("Negut Ovidiu",9,975.50,"Necunoscut");
...
///Persoanele care au realizat rezervarea

    std::cout<<"Proprietarii rezervarilor valide:"<<'\n';
    std::cout<<'\n';
    for(int i=0;i<numar_rez_valide;i++){
        std::cout<<"Rezervarea cu id ul "<<rezervari_valide[i].get_id_rezervare()<<":";
        rezervari_valide[i].proprietar_rezervare();
        std::cout<<'\n';
    }
    std::cout<<"----------------------\n";
